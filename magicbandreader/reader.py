...
@click.command(
    context_settings=dict(max_content_width=500, show_default=True, auto_envvar_prefix="MR", ignore_unknown_options=True, allow_extra_args=True)
)
@click.option(
    "-b",
    "--brightness-level",
...
@click.option("-p", "--permission", default="Open Door", help="The name of the permission to validate before authorizing.", show_envvar=True)
@click.option(
    "-s",
    "--sound-dir",
    default="/sounds",
...
@click.option("-t", "--reader-type", default="mfrc522", help="The type of RFID reader implementation to use.", show_envvar=True)
@click.option(
    "-u",
    "--api-url",
...
def main(**config):
    ...
    logging.basicConfig(
        level=getattr(logging, ctx.log_level.upper()), format="%(asctime)s %(levelname)s %(pathname)s (line: %(lineno)d): %(message)s"
    )
